...
var FormatPlugin = Plugin.sub('FormatPlugin', {
	...
	doJob: function doJob( job ) {
		console.log('DOING AN EXAMPLE JOB');
			...
			//console.log('type', job.get('spreadsheet'));
		setTimeout(function(){
			job.set('type', job.get('spreadsheet'));
			job.set('body', '<h1>'+ job.get('sheet') +'</h1>');
			job.succeed();
...
module.exports = new FormatPlugin();
