...
function setCachingHeaders(req, resp, doc) {
	...
	if ( !resp.get('Cache-Control') ) {
		resp.set( 'Cache-Control', 'public, s-maxage=' + sMaxAge + ', max-age=' + Math.min(cacheLength, year) );
...
function sendError( err, a, b ) {
	...
	console.error('Error', 'code=' + err.code, 'requestId=' + this.req.requestId, 'message=' + err.message, 'key=' + key);
