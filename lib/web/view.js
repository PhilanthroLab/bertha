...
function sendJSON( doc ) {
	...
	if ( !this.get('Cache-Control') ) {
		this.set( 'Cache-Control', 'public, proxy-revalidate, s-maxage=' + sMaxAge + ', max-age=' + Math.min(cacheLength, year) );
		this.set( 'Surrogate-Control', 'max-age=' + sMaxAge);
	}

	if ( !this.get('ETag') && doc.etag ) {
		this.set( 'ETag', doc.etag );
	}

	if (!this.get('Last-Modified') && doc.lastModified) {
		this.set( 'Last-Modified', doc.lastModified );
	}
	...
	if (type === 'json') {
		...
		if ( typeof callbackName === 'string' && /[A-Za-z0-9_\.]+/.test(callbackName) ) {
			...
			this.set('Content-Type', responseTypes[type]);
	} else {
			this.set('Content-Type', responseTypes.text);
	}
