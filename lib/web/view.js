...
function sendJSON( doc ) {
	...
	if ( !this.get('Cache-Control') ) {
		if ( doc.etag ) {
			...
			this.set( 'Cache-Control', ('public, proxy-revalidate, max-age=' + Math.min(cacheLength, year).toString() + ', s-maxage=' + Math.min(cacheLength, 30).toString()) );
		} else {
			this.set( 'Cache-Control', 'public, proxy-revalidate, max-age=0, s-maxage=0' );
		}
	} else if ( !this.get('ETag') && doc.etag ) {
		this.set( 'ETag', doc.etag );
	}
