...
Job.prototype.etag = function etag() {
	var body = this._attributes.body || '',
		len = Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body),
		result = '';

	if (  len > 1024 ) {
		result = '"' + crc32.signed( body ) + '"';
	}
...
Job.prototype.succeed = function succeed() {
	...
	try {
		...
		// TODO make sure values in the headers obj are only strings
		var headers = {cache:this.get('cache'),lastModified:this.get('lastModified'),etag:this.get('etag')};
...
Job.prototype.fail = function fail( error ) {
	console.log('Job Failed', error.message, error.code, error.stack);
